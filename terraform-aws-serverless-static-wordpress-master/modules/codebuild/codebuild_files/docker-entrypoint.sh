...
# echo $(curl ${ECS_CONTAINER_METADATA_URI_V4}/task || true)
# Get IPv4 of Running container
container_ip=$(curl ${ECS_CONTAINER_METADATA_URI_V4}/task | jq -r '.Containers[0].Networks[0].IPv4Addresses[0]') || true
echo "Private IP is: $container_ip"

# Look up Public IP from network interface
public_ip=$(aws ec2 describe-network-interfaces --filters Name=addresses.private-ip-address,Values=$container_ip --query 'NetworkInterfaces[0].Association.PublicIp' --region $WPSTATIC_REGION --output=text) || true
